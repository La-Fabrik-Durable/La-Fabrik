...
import { useGLTF, OrthographicCamera, MapControls, Line } from '@react-three/drei';
...
const EditorScene: React.FC<EditorSceneProps> = ({
  ...
  useFrame(() => {
    ...
    const sphereIntersect = intersects.find((item) => item.object.name && item.object.name.startsWith('waypoint-'));
    if (sphereIntersect) {
      const nodeId = Number(sphereIntersect.object.name.replace('waypoint-', ''));
    ...
    const terrainIntersect = intersects.find((item) => item.object.name && !item.object.name.startsWith('waypoint-'));
    const activeTerrainIntersect = terrainIntersect || intersects.find((item) => !item.object.name);
    ...
    if (activeTerrainIntersect && activeTerrainIntersect.point) {
      ...
      if (activeDragId !== null && rubberLineRef.current) {
        ...
        const startNode = waypointsRef.current.find((w) => w.id === activeDragId);
        if (startNode) {
      ...
      {lines.map(([start, end, idA, idB]) => {
        ...
        return (
          <Line
            ...
            onClick={(e) => {
              e.stopPropagation();
              console.log(`[Lien 3D] Sélectionné: Point ${idA} <-> Point ${idB}`);
...
export const WaypointEditorPage: React.FC = () => {
  ...
  const [selectedConnection, setSelectedConnection] = useState<{ idA: number; idB: number } | null>(null);
  const [hoveredConnection, setHoveredConnection] = useState<{ idA: number; idB: number } | null>(null);

  // Helper function to handle connection selection and reset node selection
  const handleSelectConnection = (conn: { idA: number; idB: number } | null) => {
    if (conn) {
      console.log(`[Sélection] Liaison active sélectionnée : Point ${conn.idA} <-> Point ${conn.idB}`);
  ...
  const [activeConnectionStartId, setActiveConnectionStartId] = useState<number | null>(null);

  // Load from localstorage on mount
  useEffect(() => {
    console.log("[Initialisation] Chargement des waypoints depuis localStorage...");
    const saved = localStorage.getItem('la-fabrik-waypoints');
    if (saved) {
      try {
        const list = JSON.parse(saved);
        console.log(`[Initialisation] ${list.length} waypoints chargés avec succès !`);
        setWaypoints(list);
      } catch (e) {
        console.error('[Initialisation] Erreur de parsing du stockage local', e);
      }
    } else {
      console.log("[Initialisation] Aucun point enregistré en localStorage. Démarrage à vide.");
    }
  }, []);
  ...
  const deleteSelectedConnection = (idA: number, idB: number) => {
    console.log(`[Liaisons] Suppression définitive du lien : Point ${idA} <-> Point ${idB}`);
    setWaypoints((currentWaypoints) => {
      const updatedList = currentWaypoints.map((wp) => {
        if (wp.id === idA) {
          return { ...wp, connections: wp.connections.filter((cId) => cId !== idB) };
        }
        if (wp.id === idB) {
          return { ...wp, connections: wp.connections.filter((cId) => cId !== idA) };
        }
        return wp;
      });
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        if (selectedId !== null) {
          console.log(`[Hotkey] Touche de suppression activée sur le Point sélectionné : ID = ${selectedId}`);
          handleDeleteNode(selectedId);
        } else if (selectedConnection !== null) {
          console.log(`[Hotkey] Touche de suppression activée sur la Liaison sélectionnée : ${selectedConnection.idA} <-> ${selectedConnection.idB}`);
  ...
  const handleTerrainClick = (point: THREE.Vector3) => {
    if (isConnectingMode) {
      console.log("[Mode Connexion] Clic sur terrain vide. Annulation du mode liaison.");
    ...
    setWaypoints((currentWaypoints) => {
      const nextId = currentWaypoints.length > 0 ? Math.max(...currentWaypoints.map((w) => w.id)) + 1 : 1;
      ...
      console.log(`[Création] Nouveau Point déposé : ID = ${nextId} | Coordonnées : (${newWp.x}, ${newWp.y}, ${newWp.z})`);
  ...
  const handleSelectNode = (id: number) => {
    ...
    if (isConnectingMode && activeConnectionStartId !== null) {
      if (activeConnectionStartId === id) {
        console.log("[Mode Connexion] Tentative de liaison sur soi-même. Annulation.");
        setIsConnectingMode(false);
        setActiveConnectionStartId(null);
        return;
      }

      console.log(`[Mode Connexion] Création manuelle d'un lien : Point ${activeConnectionStartId} <-> Point ${id}`);
    ...
    setWaypoints((currentWaypoints) => {
      const updatedList = currentWaypoints.map((wp) => {
        if (wp.id === idA) {
          ...
          console.log(`[Liaisons] Point ${idA} : ${alreadyLinked ? 'Suppression' : 'Ajout'} de la liaison vers Point ${idB}`);
        ...
        if (wp.id === idB) {
          ...
          console.log(`[Liaisons] Point ${idB} : ${alreadyLinked ? 'Suppression' : 'Ajout'} de la liaison vers Point ${idA}`);
          const conns = alreadyLinked
  ...
  const handleGlobalPointerUp = () => {
    if (dragStartNodeId !== null) {
      if (hoveredNodeId !== null && hoveredNodeId !== dragStartNodeId) {
        console.log(`[Drag&Drop] Relâchement sur le Point existant : ID = ${hoveredNodeId}. Création/Toggling du lien.`);
        ...
        setWaypoints((currentWaypoints) => {
          ...
          console.log(`[Drag&Drop] Relâchement sur zone vide. Création automatique du Point ${nextId} aux coordonnées (${newWp.x}, ${newWp.y}, ${newWp.z}) et liaison mutuelle avec le Point ${dragStartNodeId}`);

          const updatedList = currentWaypoints.map((wp) => {
            if (wp.id === dragStartNodeId) {
              return {
                ...wp,
                connections: wp.connections.includes(nextId) ? wp.connections : [...wp.connections, nextId],
              };
            }
            return wp;
          });
          ...
          localStorage.setItem('la-fabrik-waypoints', JSON.stringify(finalList));
          setTimeout(() => {
            setSelectedConnection(null);
            setSelectedId(nextId);
          }, 0);
  ...
  const handleDeleteNode = (id: number) => {
    console.log(`[Suppression] Action de suppression définitive du Point : ID = ${id}`);
    setWaypoints((currentWaypoints) => {
    ...
    setSelectedId((currentSelected) => (currentSelected === id ? null : currentSelected));
  };

  // Connect Mode Trigger
  const startConnecting = (id: number) => {
    console.log(`[Mode Connexion] Démarrage mode connexion manuelle depuis Point ID = ${id}`);
  ...
  const handleClearAll = () => {
    if (window.confirm('Voulez-vous vraiment TOUT supprimer ? Cette action est irréversible.')) {
      console.log("[Action] Suppression complète et définitive de tous les points de la carte.");
      saveWaypoints([]);
      setSelectedId(null);
      setSelectedConnection(null);
    }
    ...
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerControls}>
          <button style={styles.secondaryButton} onClick={handleCopyToClipboard}>
        ...
        <aside style={styles.sidebar}>
          ...
          <div style={styles.nodesList}>
            {waypoints.length === 0 ? (
              <div style={styles.emptyState}>
                ...
                <p style={styles.emptyText}>Cliquez sur le terrain pour placer votre premier point.</p>
              </div>
            ) : (
              waypoints.map((wp) => (
                <div 
          ...
          {selectedNode && (
            <div style={styles.detailsCard}>
              <h3 style={styles.detailsTitle}>Détails Waypoint {selectedNode.id}</h3>
              <div style={styles.detailsGrid}>
                <div style={styles.detailsRow}>
                  <strong>Pos X:</strong> <span>{selectedNode.x}</span>
                </div>
                <div style={styles.detailsRow}>
                  <strong>Pos Y:</strong> <span>{selectedNode.y} (Hauteur)</span>
                </div>
                <div style={styles.detailsRow}>
              ...
              <div style={styles.detailsActions}>
                <button 
                  style={styles.connectButton(isConnectingMode && activeConnectionStartId === selectedNode.id)}
          ...
          {selectedConnection && (
            <div style={styles.detailsCard}>
              ...
              <div style={{ ...styles.detailsGrid, marginBottom: '1.25rem' }}>
                <div style={styles.detailsRow}>
                  <strong>Point A:</strong> <span>ID {selectedConnection.idA}</span>
                </div>
                <div style={styles.detailsRow}>
                  <strong>Point B:</strong> <span>ID {selectedConnection.idB}</span>
                </div>
                <div style={styles.detailsRow}>
              ...
              <button 
                style={styles.deleteButton} 
                onClick={() => deleteSelectedConnection(selectedConnection.idA, selectedConnection.idB)}
              >
        ...
        >
          ...
          {isConnectingMode && (
            <div style={styles.connectingBanner}>
              <Info size={16} /> Mode Connexion Actif : Cliquez sur le deuxième waypoint pour lier le point {activeConnectionStartId}.
            </div>
          )}

          {dragStartNodeId !== null && (
            <div style={styles.connectingBanner}>
              <Link2 size={16} /> Relier le point {dragStartNodeId}... Glissez et relâchez sur un autre point.
            </div>
          )}
          ...
          <Canvas shadows>
            ...
            <directionalLight position={[50, 200, 50]} intensity={2.0} castShadow />
            
            {/* Locked Orbit/Map controls (Locked rotation for strict top-down, disabled during link dragging to prevent panning) */}
            <MapControls enableRotate={false} enabled={dragStartNodeId === null} />
